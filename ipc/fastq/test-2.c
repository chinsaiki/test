...
//        usleep(100000);
        ...
        FastQTrySendStats(parg->srcModuleId, NODE_1, &addr, sizeof(unsigned long));
...
void *dequeue_task(void*arg) {
    ...
    FastQRecvStats( NODE_1, handler_test_msg);
    pthread_exit(NULL);
}

int sig_handler(int signum) {

    FastQDumpStats(NULL, NODE_1);
    exit(1);
}
...
int main()
{
    ...
    FastQCreateModuleStats(NODE_1, max_msg, sizeof(unsigned long), __FILE__, __func__, __LINE__);
    FastQCreateModuleStats(NODE_2, max_msg, sizeof(unsigned long), __FILE__, __func__, __LINE__);
    FastQCreateModuleStats(NODE_3, max_msg, sizeof(unsigned long), __FILE__, __func__, __LINE__);
    FastQCreateModuleStats(NODE_4, max_msg, sizeof(unsigned long), __FILE__, __func__, __LINE__);
