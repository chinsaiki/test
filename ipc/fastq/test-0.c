...
/* 队列发送 NODE， 将 FastQCreate 接口参数说明 */
#define NODE_1 1
#define NODE_2 2
...
/* 队列句柄 */
struct FastQContext ctx1;

/* 发送消息主任务 */
void *enqueue_task(void*arg) {
    /* 获取句柄 */
    struct FastQContext *ctx = (struct FastQContext *)arg;
    int i =0;
    test_msgs_t *pmsg;
    ...
    while(1) {
        ...
        FastQSendto(ctx, NODE_1, NODE_2, &addr, sizeof(unsigned long));
    }
    pthread_exit(NULL);
}
...
void *dequeue_task(void*arg) {
    ...
    while(1) {
        ...
        FastQRecvrom(ctx, NODE_1, NODE_2, &addr, &sz);
